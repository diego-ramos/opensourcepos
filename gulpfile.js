...
gulp.task('compress', function () {
    const sources = ['app*/**/*', 'public*/**/*', 'vendor*/**/*', '*.md', 'LICENSE', 'docker*', 'Dockerfile', '**/.htaccess', 'writable*/**/*', '!writable/logs/*.log', '!writable/debugbar/*.json', '.env.example'];
    gulp.src(sources, { allowEmpty: true, encoding: false }).pipe(tar('opensourcepos.tar')).pipe(gulp.dest('dist'));
